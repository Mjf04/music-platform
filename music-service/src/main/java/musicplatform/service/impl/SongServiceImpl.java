...
import musicplatform.enumeration.LikeStatusEnum;
...
import musicplatform.mapper.UserFavoriteMapper;
...
import musicplatform.service.ISongService;
...
public class SongServiceImpl extends ServiceImpl<SongMapper, Song> implements ISongService {
    ...
    public Result<List<SongVO>> getRecommendedSongs(HttpServletRequest request) {
        ...
        // 如果用户收藏的歌曲没有风格信息，返回随机歌曲
        if (favoriteStyleIds.isEmpty()) {
            return Result.success(songMapper.getRandomSongsWithArtist());
        }
        ...
        Set<Long> favoriteSet = new HashSet<>(favoriteSongIds);
        for (SongVO song : recommendedSongs) {
            song.setLikeStatus(
                    favoriteSet.contains(song.getSongId())
    ...
    @Override
    @CacheEvict(cacheNames = "songCache", allEntries = true)
    public Result updateSongLyric(Long songId, String lyric) {
        Song song = songMapper.selectById(songId);
        if (song == null) {
            return Result.error(MessageConstant.SONG + MessageConstant.NOT_FOUND);
        }
