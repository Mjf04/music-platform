...
import musicplatform.model.dto.*;
...
import musicplatform.model.vo.UserManagementVO;
import musicplatform.result.PageResult;
import musicplatform.result.Result;
import musicplatform.service.*;
import musicplatform.util.BindingResultUtil;
...
public class AdminController {
    ...
    public Result updateAdminInfo(@RequestBody @Valid AdminUpdateDTO adminUpdateDTO, BindingResult bindingResult) {
        String errorMessage = BindingResultUtil.handleBindingResultErrors(bindingResult);
        if (errorMessage != null) {
            return Result.error(errorMessage);
        }
        return adminService.updateAdminInfo(adminUpdateDTO);
    }

    /**
     * 更新当前管理员头像
     */
    @PatchMapping("/updateAdminAvatar")
    public Result updateAdminAvatar(@RequestParam("avatar") MultipartFile avatar) {
        String avatarUrl = minioService.uploadFile(avatar, "admins");
        return adminService.updateAdminAvatar(avatarUrl);
    }

    /**
     * 修改当前管理员密码
     */
    @PatchMapping("/updateAdminPassword")
    public Result updateAdminPassword(@RequestBody @Valid UserPasswordDTO adminPasswordDTO,
        ...
        return adminService.updateAdminPassword(adminPasswordDTO, token);
    }
    ...
    public Result updateSongLyric(@PathVariable("id") Long songId, @RequestBody Map<String, String> requestBody) {
        String lyric = requestBody.get("lyric");
        return songService.updateSongLyric(songId, lyric);
    }
